...
- [Build the About and contact/social pages](issues/07-about-contact-pages.md) — two-column About layout, brutalist type, animated social links (ArtStation, Instagram, email); edit `src/pages/about.astro` to personalise; `b0904e9`.
- [Set up GitHub Pages deployment via Actions](issues/09-github-pages-deploy.md) — `.github/workflows/deploy.yml`: push-to-main triggers build + deploy via upload-pages-artifact + deploy-pages; Node 22, npm ci, concurrency lock; first push sent, deploy in progress at https://Ouroborusss.github.io/portfolio.
